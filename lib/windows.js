...
function runAHKScript(script) {
	...
	if (ahk.ahkReady()) ahk.ahkTerminate(-1);
	if (doneTimer !== null) {
	...
	return new Promise( (resolve) => {
		if (ahk.ahkReady()) resolve({url: '', browser: ''});
		...
		doneTimer = setInterval(() => {
			...
			const bFinished = ahkGetVar('bFinished');
			if (bFinished == 1) {
				clearInterval(doneTimer);
				doneTimer = null;
				const sURL = ahkGetVar('sURL');
				const sClass = ahkGetVar('sClass');
				ahk.ahkTerminate(1);
