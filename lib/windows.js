...
const user32 = new ffi.Library('User32.dll', {
	...
	GetWindowTextW: ['int', ['pointer', 'uchar *', 'int']],
...
const psapi = new ffi.Library('kernel32', {
	// https://docs.microsoft.com/en-us/windows/desktop/api/psapi/nf-psapi-getprocessmemoryinfo
	K32GetProcessMemoryInfo: ['int', ['pointer', ProcessMemoryCountersPointer, 'uint32']]
});
...
const kernel32 = new ffi.Library('kernel32', {
	...
	QueryFullProcessImageNameW: ['int', ['pointer', 'uint32', 'uchar *', 'pointer']]
});
...
function getProcessPath(processHandle) {
	...
	const pathLengthBytes = 32767;
	// Allocate a buffer to store the path of the process
	const processFileNameBuffer = Buffer.allocUnsafeSlow(pathLengthBytes * wchar.size);
	// Create a buffer containing the allocated size for the path, as a buffer as it must be writable
	const processFileNameSizeBuffer = ref.alloc('uint32', pathLengthBytes);

	var processPath = "";
	// Write process file path to buffer
	if (kernel32.QueryFullProcessImageNameW(processHandle, 0, processFileNameBuffer, processFileNameSizeBuffer)) {
		// Get process file path as a string
		if (!ref.isNull(processFileNameBuffer)) {
			processPath = ref.reinterpretUntilZeros(processFileNameBuffer, wchar.size).toString('ucs2');
		}
	}
...
function windows() {
	...
	const windowTextBuffer = Buffer.allocUnsafeSlow((windowTextLength + 1) * wchar.size);

	var windowTitle = "";
	// Write the window text to the buffer (it returns the text size, but it's not used here)
	if (windowTextLength && user32.GetWindowTextW(activeWindowHandle, windowTextBuffer, windowTextLength + 1)) {
		// The text as a JavaScript string
		if (!ref.isNull(windowTextBuffer)) {
			windowTitle = ref.reinterpretUntilZeros(windowTextBuffer, wchar.size).toString('ucs2');
		}
	}
	...
	const getProcessMemoryInfoResult = psapi.K32GetProcessMemoryInfo(processHandle, memoryCounters.ref(), ProcessMemoryCounters.size);
...
const win_path = path.join(base_path, 'platform/win/');
const arch_path = path.join(win_path, process.arch);

kernel32.SetDllDirectoryW(Buffer.from(arch_path + '\0', 'ucs2'));

const ahk = new ffi.Library(path.join(arch_path, 'AutoHotkey.dll'), {
	ahkdll: ['pointer', ['uchar *', 'uchar *', 'uchar *']],
	ahkReady: ['int', []],
	ahkgetvar: ['uchar **', ['uchar *', 'uint']],
	ahkTerminate: ['int', ['int']],
	ahkFunction: ['int', ['uchar *']]
});

kernel32.SetDllDirectoryW(ref.NULL);

function ahkGetVar(variable) {
	const buf = ahk.ahkgetvar(Buffer.from(variable + '\0', 'ucs2'), 0);
	if (ref.isNull(buf)) {
		return "";
	}
	return buf.reinterpretUntilZeros(wchar.size).toString('ucs2');
}
...
function stopAHKScript() {
	if (ahk.ahkReady()) {
		ahk.ahkFunction(Buffer.from('StopScript\0', 'ucs2'));
		ahk.ahkTerminate(100);
	}
}

function runAHKScript(script) {
	// Is the script not done yet?
	stopAHKScript();
	if (doneTimer !== null) {
	...
	return new Promise( (resolve) => {
		...
		ahk.ahkdll(Buffer.from(script + '\0', 'ucs2'), null, null);
		doneTimer = setInterval(() => {
			...
			bFinished = ahkGetVar('bFinished');
			if (bFinished == 1) {
				clearInterval(doneTimer);
				doneTimer = null;
				sURL = ahkGetVar('gURL');
				sClass = ahkGetVar('gClass');
				stopAHKScript();
				resolve({url: sURL, browser: sClass});
			}
		}, 250);
	});
}
...
const getWinURL = () => {
	return new Promise( (resolve) => {
		...
		if (result !== undefined) {
			try {
				runAHKScript(path.join(win_path, 'get_url.ahk')).then( (info) => {
					resolve(Object.assign({}, result, info));
				});
			} catch (e) {
				resolve(Object.assign({}, result, {url: '', browser: ''}));
			}
		} else {
			resolve(result);
		}
...
module.exports = async () => { return await getWinURL() };
module.exports.sync = getWinURL;
