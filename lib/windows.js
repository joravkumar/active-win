...
function getProcessPath(processHandle) {
	...
	if (kernel32.QueryFullProcessImageNameW(processHandle, 0, processFileNameBuffer, processFileNameSizeBuffer)) {
		...
		if (!ref.isNull(processFileNameBuffer)) {
			processPath = processFileNameBuffer.reinterpretUntilZeros(wchar.size).toString('ucs2');
		}
	}
...
function windows() {
	...
	if (windowTextLength && user32.GetWindowTextW(activeWindowHandle, windowTextBuffer, windowTextLength + 1)) {
		...
		if (!ref.isNull(windowTextBuffer)) {
			windowTitle = windowTextBuffer.reinterpretUntilZeros(wchar.size).toString('ucs2');;
		}
	}
