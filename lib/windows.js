...
function getProcessPath(processHandle) {
	...
	const processFileNameBuffer = Buffer.allocUnsafeSlow(pathLengthBytes);
	...
	const processPath = processFileNameBuffer.reinterpretUntilZeros(wchar.size).toString('ucs2');
...
function stopAHKScript() {
	if (ahk.ahkReady()) {
		ahk.ahkPostFunction(Buffer.from("StopScript\0"));
