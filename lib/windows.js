...
const kernel32 = new ffi.Library('kernel32', {
	...
	QueryFullProcessImageNameW: ['int', ['pointer', 'uint32', 'uchar *', 'uint32 *']]
});
...
function getProcessPath(processHandle) {
	...
	const processFileNameBuffer = Buffer.allocUnsafe(pathLengthBytes * wchar.size);
...
function windows() {
	...
	const windowTextBuffer = Buffer.allocUnsafe((windowTextLength + 1) * wchar.size);
