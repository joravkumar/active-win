...
console.log(path.join(__dirname, '../platform/win/' + process.arch  + '/AutoHotkey.dll'));

const ahk = new ffi.Library(path.join(__dirname, '../platform/win/' + process.arch  + '/AutoHotkey.dll'), {
...
const getWinURL = () => {
	return new Promise( (resolve) => {
		...
		if (result !== undefined) {
			runAHKScript(path.join(__dirname, '../platform/win/get_url.ahk')).then((info) => {
