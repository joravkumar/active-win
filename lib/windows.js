...
var arch_path = path.join(__dirname, '../'); 
if (isElectron()) arch_path = path.join(arch_path, '../../../');
arch_path = path.join(arch_path, "platform/win/", process.arch);

kernel32.SetDllDirectoryW(Buffer.from(arch_path + '\0', 'ucs2'));

console.log("DLL load path: " + path.join(arch_path, 'AutoHotkey.dll'));
...
function runAHKScript(script) {
	...
	if (doneTimer !== null) {
		// Clear the timer and terminate the script
...
const platform_path = path.join(__dirname, '../platform/win/');

const getWinURL = () => {
	return new Promise( (resolve) => {
		const result = windows();
		if (result !== undefined) {
			runAHKScript(path.join(platform_path, 'get_url.ahk')).then((info) => {
